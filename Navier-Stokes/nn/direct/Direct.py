...
prefix = "../"
θ = np.load(prefix+"NS_theta_" + str(N_θ) + ".npy")   
κ = np.load(prefix+"NS_omega_" + str(N_θ) + ".npy")
...
κ_pred = np.zeros((N_x, N_y, N_data))

for i in range(N_data):
    d_range = range(i*N_tot, (i + 1)*N_tot)
    κ_pred[:,:,i] = output_train[d_range].reshape(N_x, N_y)/coeff_scale
...
for i in range(N_data):
    d_range = range(i*N_tot, (i + 1)*N_tot)
    κ_pred[:,:,i] = output_test[d_range].reshape(N_x, N_y)/coeff_scale

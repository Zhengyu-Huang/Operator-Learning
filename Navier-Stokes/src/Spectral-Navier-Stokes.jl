...
# we can also use cos(k⋅x)/√2π and sin(k⋅x)/√2π as basis (L2 norm is 1) for zero-mean real fields
# with (kx + ky > 0 || (kx + ky == 0 && kx > 0)) 
# θ = [ak;bk]
...
function Initial_ω0_KL(mesh::Spectral_Mesh, θ::Array{Float64,1}, seq_pairs::Array{Int64,2}; τ = 0.0)
    ...
    # basis are cos(k⋅x)/√2π and sin(k⋅x)/√2π, k!=(0,0), zero mean.
    # ω = ∑ ak cos(k⋅x)/√2π|k|^2 + ∑ bk sin(k⋅x)/√2π|k|^2,    &  kx + ky > 0 or (kx + ky = 0 and kx > 0) 
    #   = ∑ (ak/(2√2π|k|^2) - i bk/(2√2π|k|^2) )e^{ik⋅x} + (ak/(2√2π|k|^2) + i bk/(2√2π|k|^2) )e^{-ik⋅x}

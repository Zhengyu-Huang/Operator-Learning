...
for i in range(M//2):
    print("i / N = ", i, " / ", M//2)
    K_train = y_normalizer.decode(model(x_train[i:i+1, :, :, :].to(device))).detach().cpu().numpy()[0,:,:,0]
    
    rbs = interpolate.RectBivariateSpline(xgrid, xgrid, K_train)
    K_train_coarse = rbs.ev(XY[:, 0], XY[:, 1])

    rel_err_nn_train[i] =  np.linalg.norm(K_train_coarse - K_org[i, :])/np.linalg.norm(K_org[i, :])
...
# plt.ylabel('y')
# plt.tight_layout()
# plt.savefig('worst_case_train_NN%d.png' %(width), pad_inches=3)
# plt.close()
...
for i in range(M-M//2):
    print("i / N = ", i, " / ", M-M//2)
    K_test = y_normalizer.decode(model(x_test[i:i+1, :, :, :].to(device))).detach().cpu().numpy()[0,:,:,0]
    
    rbs = interpolate.RectBivariateSpline(xgrid, xgrid, K_test)
    K_test_coarse = rbs.ev(XY[:, 0], XY[:, 1])
    
    rel_err_nn_test[i] =  np.linalg.norm(K_test_coarse - K_org[i + M//2, :])/np.linalg.norm(K_org[i + M//2, :])
...
# plt.ylabel('y')
# plt.tight_layout()
# plt.savefig('worst_case_test_NN%d.png' %(width), pad_inches=3)
# plt.close()
...
plt.tight_layout()
plt.savefig('NN_errors%d.png' %(width),pad_inches=3)
plt.close()

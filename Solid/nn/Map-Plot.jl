...
function prediction_plot(nn_name, ntrain, width, ind)
    ...
    vmin, vmax = outputs[:,:,ind].min(), outputs[:,:,ind].max()
    ax[0].pcolormesh(X, Y, inputs[:, :, ind],    shading="gouraud")
    ax[1].pcolormesh(X, Y, outputs[:, :, ind],   shading="gouraud", vmin=vmin, vmax =vmax)
    ax[2].pcolormesh(X, Y, outputs[:, :, ind+3], shading="gouraud", vmin=vmin, vmax =vmax)

...
function map_plot(prefix = "../src/", inds = [1,11])
    ...
    for ind in inds
        input_plot(inputs[:, ind], "Advection-map-input-$(ind).pdf")
        output_plot(outputs[:, ind], "Advection-map-output-$(ind).pdf")
    end
end
...
function prediction_plot(nn_name, ntrain, width, ind)
    ...
    fig.tight_layout()
    fig.savefig("Advection-" * err[ind] * "_" * nn_name * "_" * string(ntrain) * "_" * string(width)* ".pdf")

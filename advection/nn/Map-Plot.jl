...
function meshgrid(xin, yin)
  ...
  function input_plot(data, file_name)
      ...
      plot(xx, data, "--o", fillstyle="none",markersize=2, color="#a1a1a1")
      ax = gca()
      ax.set_title(L"u_0",pad = 10,fontsize=28,color=lbl)   
      ax.spines["top"].set_visible(false)
      ax.spines["right"].set_visible(false)
      ax.spines["left"].set_color(tk)
      ax.spines["bottom"].set_color(tk)
      ax[:set_xlim]([0,1])
      ax.set_xlabel(L"x",labelpad=10,fontsize=20,color=lbl)
      ax[:xaxis][:set_tick_params](colors=tk)
      ax[:yaxis][:set_tick_params](colors=tk)
  ...
  function output_plot(data, file_name)
      ...
      plot(xx, data, "--o", fillstyle="none",markersize=2, color="#a1a1a1")
      ax = gca()
      ax.set_title(L"u(T)",pad = 10,fontsize=28,color=lbl)   
      ax.spines["top"].set_visible(false)
      ax.spines["right"].set_visible(false)
      ax.spines["left"].set_color(tk)
      ax.spines["bottom"].set_color(tk)
  ...
  function map_plot(prefix = "../src/", inds = [18])
      ...
      for ind in inds
          input_plot(inputs[:, ind], "Advection-low-map-input-$(ind)-"*coloroption*".pdf")
          output_plot(outputs[:, ind], "Advection-low-map-output-$(ind)-"*coloroption*".pdf")
      end
  end
  ...
  for i = 1:4
      ...
      ax[1,i].set_title(nns[i],pad = 2,color=lbl)
      ax[3,i].set_xlabel(L"x",labelpad=1,color=lbl)
  
      for j = 1:3
          ax[j,i].spines["top"].set_visible(false)
          ax[j,i].spines["right"].set_visible(false)
          ax[j,i].spines["left"].set_color(tk)
          ax[j,i].spines["left"].set_linewidth(0.3)
          ax[j,i].spines["bottom"].set_color(tk)
          ax[j,i].spines["bottom"].set_linewidth(0.3)
          ax[j,i][:xaxis][:set_tick_params](colors=tk,width=0.3)
          ax[j,i][:yaxis][:set_tick_params](colors=tk,width=0.3)
      end
  end
  ax[1,1].set_ylabel(L"u_0",color=lbl)
  ax[2,1].set_ylabel("True "*L"u(T)",color=lbl)
  ax[3,1].set_ylabel("Predicted "*L"u(T)",color=lbl)
  plt.subplots_adjust(left = 0.08, right = 0.98, bottom = 0.1,top=.9,hspace=0.1,wspace=0.1)
  plt.savefig("Advection-dc-low-medians-"*coloroption*".pdf")
  ...
  # plt.tight_layout()
  plt.subplots_adjust(left = 0.08, right = 0.98, bottom = 0.1,top=.9,hspace=0.1,wspace=0.1)
  plt.savefig("Advection-dc-low-worst-"*coloroption*".pdf")
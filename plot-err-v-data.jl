...
#         ax[i,1].plot(N_Width,PCA_Data[(j+3)*5+1:(j+3)*5+5,5,i],color=colors[1], linestyle=linestyle[j], marker = markers[j], fillstyle="none",label =  L"N = "*string(Int(N_Data[j])))
#         ax[i,2].plot(N_Width,DeepONet_Data[(j+3)*5+1:(j+3)*5+5,5,i],color=colors[2], linestyle=linestyle[j], marker = markers[j], fillstyle="none",label =  L"N = "*string(Int(N_Data[j])))
#         if j < 4
#             ax[i,3].plot(N_Width,PARA_Data[(j+3)*5+1:(j+3)*5+5,5,i],color=colors[3], linestyle=linestyle[j], marker = markers[j], fillstyle="none",label=L"N = "*string(Int(N_Data[j])))
#         end
#         ax[i,4].plot(N_Df,FNO_Data[(j+3)*5+1:(j+3)*5+5,5,i],color=colors[4], linestyle=linestyle[j], marker = markers[j], fillstyle="none",label=L"N = "*string(Int(N_Data[j])))
...
for i = 1:4 # test problems
    for j = 1:4 # plot columns
        ...
        ax[i,j].semilogx(PCA_Data[(j+3)*5+1:(j+3)*5+5, 3, i], PCA_Data[(j+3)*5+1:(j+3)*5+5, 5, i], color = colors[1], linestyle=linestyle[i], marker = markers[1], fillstyle="none",      label =  nns[1]  )
        ax[i,j].semilogx(DeepONet_Data[(j+3)*5+1:(j+3)*5+5, 3, i], DeepONet_Data[(j+3)*5+1:(j+3)*5+5, 5, i], color = colors[2], linestyle=linestyle[i], marker = markers[2], fillstyle="none", label =  nns[2]  )
        if j < 4
            ax[i,j].semilogx(PARA_Data[(j+3)*5+1:(j+3)*5+5, 3, i], PARA_Data[(j+3)*5+1:(j+3)*5+5, 5, i], color = colors[3], linestyle=linestyle[i], marker = markers[3], fillstyle="none",     label =  nns[3]  )
        end
        ax[i,j].semilogx(FNO_Data[(j+3)*5+1:(j+3)*5+5, 3, i], FNO_Data[(j+3)*5+1:(j+3)*5+5, 5, i], color = colors[4], linestyle=linestyle[i], marker = markers[4], fillstyle="none",      label =  nns[4]  )
        # if i == 1
        #     ax[i,j].title.set_text("N = "*string(Int(FNO_Data[(j+3)*5+1, 1, i])))   
        # end

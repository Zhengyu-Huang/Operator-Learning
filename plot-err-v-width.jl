# script to plot err vs. network width 
...
for i = 1:4 # test problems
    for j = 1:4 # loop N 
        ...
        if i == 4
            if j == 4
                ax[i,j].set_xlabel(latexstring("Lifting dimension "*L"d_f"),labelpad=2,color=lbl)
            else
                ax[i,j].set_xlabel(latexstring("Network width ",L"w"),labelpad=2,color=lbl)
            end
        end
